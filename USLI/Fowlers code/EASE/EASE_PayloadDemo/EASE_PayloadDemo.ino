/*-------------------------------------------------------------
  ...
  Program Flow:
    Respond to manual commands in packet until armed
    Once armed, monitor incoming packets for SOAR signal to run deployment
    Set encoder target for deployment
    Run motor using proportional control until target reached
    Send packet indicating deployment complete
    Revert to manual control mode
  -------------------------------------------------------------*/
#define LED_1 18
#define LED_2 19
#define LIMIT_1 16
